...
def interface():
    ...
    p.add_option('--scan-type', '-s', dest='scan_type', default='all', 
    help="The type of scan to run.  Valid options are:  'all' to search \
for everything; '1' | '2' | '3 | '4' | '5' | '6' to search for only mononuc, \
dinuc, trinuc, etc.; '2+' | '3+' | '4+' | '5+' | '6+' to search for >= dinuc, \
>= trinuc, etc. or '2-4', '4-6', etc. to search within ranges. [default=all]")
...
def out(name, matches, output=None, stdout=True):
    if matches:
        for msat in matches:
